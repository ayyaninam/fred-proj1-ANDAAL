...
class StripeSCAPaymentDetailsView(CorePaymentDetailsView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # self.request.session["stripe_payment_intent_id"] = None
...
class StripeSCASuccessResponseView(CorePaymentDetailsView):
    preview = True
    template_name_preview = 'django-oscar-stripe-sca/checkout/stripe_preview.html'
    ...
    def handle_payment(self, order_number, order_total, **kwargs):
        ...
        session_id = self.request.session["stripe_session_id"]

        intent = Facade()

        pi = intent.retrieve_payment_intent(session_id)
        # details = intent.retrieve_payment_intent(pi)

        intent.capture(order_number, session_id)
